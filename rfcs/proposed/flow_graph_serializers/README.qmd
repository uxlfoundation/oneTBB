...
This document provides some analysis of the timing performance of the `rl_function_node` node type as provided in [meld-serial](https://github.com/knoepfel/meld-serial).
It analyzes a simple data flow with 7 nodes[^1].
...
[^1]: In this discussion, we do not refer to resource-limiter nodes as *nodes* but as *resource limiters*.
...
Flow Graph seem to prefer keeping some tasks on a single thread.
