...
1. Introducing the equivalent of a `resource_limiter_node` class template that, when connected with another node, ensures limited access to the resource it represents.
2. Adding a `rl_function_node` class template that allow the specification of limited resource nodes instead of (or in addition to) a `concurrency` value.
...
The node body is invoked only when the `rl_function_node` can obtain exclusive access to one of the resource handles provided by each resource-limiter node (and when a `Hits` message has been sent to it).
