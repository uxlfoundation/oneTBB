...
1. *Register their resource requirements*: Components can declare their threading needs and
   constraints.
2. *Negotiate resource allocation*: TCM mediates between components to determine optimal resource
   distribution.
...
3. *Minimally Intrusive*: Existing applications can benefit without requiring extensive
   modifications.
...
Thread Composability Manager is distributed in oneAPI packages as a binary-only, hidden component.
Its use is already integrated and supported by oneTBB and Intel's OpenMP runtime. The TCM is
installed when either or both of these libraries are selected for installation. Yet, the TCM does
not depend on the either of these parallel runtimes and can be used by other threading frameworks.
