...
* `EXAMPLES_UI_MODE` - defines the GUI mode, supported values are `gdi`, `d2d`, `con` on Windows, `x`,`con` on Linux and `mac`,`con` on macOS. The default mode is `con`.
...
* `threading-runtime` - the name of the threading runtime that will execute fractal (tbb, omp, mix).
