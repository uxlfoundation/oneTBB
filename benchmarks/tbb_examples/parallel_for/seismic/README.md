# Seismic sample
Parallel seismic wave simulation that supports mix of different parallel runtimes oneTBB and OpenMP.
There are two functions UpdateStress and UpdateVelocity and both of them have oneTBB and OpenMP based implementations.
...
* `make run_seismic` - executes the example with predefined parameters. (UpdateStress and UpdateVelocity use oneTBB implementation)
* `make perf_run_seismic` ` - executes the example with suggested parameters to measure the oneTBB performance. (UpdateStress and UpdateVelocity use oneTBB implementation)
* `make light_test_seismic` - executes the example with suggested parameters to reduce execution time. (UpdateStress and UpdateVelocity use oneTBB implementation)

### Application parameters
Usage:
```
seismic [n-of-threads=value] [n-of-frames=value] [stress-runtime=value] [velocity-runtime=value] [silent] [serial] [-h] [n-of-threads [n-of-frames]]
```
...
* `stress-runtime` - The name of the runtime that will execute UpdateStress. Possible values: tbb, omp.
* `velocity-runtime` - The name of the runtime that will execute UpdateVelocity. Possible values: tbb, omp.
