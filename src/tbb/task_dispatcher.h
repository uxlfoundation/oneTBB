...
inline d1::task* task_dispatcher::steal_or_get_critical(
    ...
    isolation_type isolation, bool critical_allowed, unsigned& stealing_hint)
{
    if (d1::task* t = a.steal_task(arena_index, random, ed, isolation, stealing_hint)) {
    ...
    isolation_type isolation, bool fifo_allowed, bool critical_allowed)
{
    ...
    unsigned stealing_hint = empty_hint;
    ...
    for (;;) {
        ...
        if (t != nullptr) {
            ...
            // Checked if there are tasks in starvation-resistant stream. Only allowed at the outermost dispatch level without isolation.
                 ...
                 && (t = steal_or_get_critical(ed, a, arena_index, tls.my_random, isolation, critical_allowed, stealing_hint))) {
