...
This section describes named requirements used in |short_name|.

A *named requirement* is a set of requirements on a type. The requirements may be syntactic or semantic.
The term *named requirement* is similar to "Requirements on types and expressions" defined
...
For example, the named requirement *sortable* could be defined as a set of requirements that allows
sorting an array of type ``T``. A type ``T`` would be *sortable* if:
...
It has the drawback of relegating important details to notational conventions. |short_name| mostly uses
pseudo-signatures because they are concise and can serve as a starting point when writing user code.

A pseudo-signature describes how |short_name| interacts with a type or a function.
...
would not compile if |short_name| attempts to pass a constant object to the function.
...
via implicit conversion of references to values and implicitly discarding the returned value
(ignored by |short_name|), it is unable to swap the actual variables passed to the function
...
For callable requirements, a pseudo-signature that uses ``operator()`` is a notation
for invocation semantics, not a requirement to provide that exact member function.
Equivalent callable forms are acceptable, including function objects, lambda expressions,
and function pointers, as long as their invocation behavior matches the requirement.

.. note::
    Starting with C++20, most |short_name| template APIs are constrained to enforce named
    requirements and provide earlier diagnostics when a type does not meet them.
