.. _parallel_deterministic_reduce_func
...
For the functional form, ``Func`` is applied to a deterministic set of subranges, and ``Reduction`` merges partial results in a deterministic order.
...
Modifying a program to use ``parallel_deterministic_reduce`` instead of ``parallel_reduce`` can be done in a few steps:

1. Change the function name.
2. If the used partitioner is not supported by ``parallel_deterministic_reduce``, change the partitioner.
3. If you notice performance degradation, a grain size for ``blocked_range`` may need to be specified or adjusted.

The example below is taken from :doc:`parallel_reduce <parallel_reduce_func>` and modified to use ``parallel_deterministic_reduce``.

.. literalinclude:: ./examples/parallel_deterministic_reduce_example.cpp
    :language: c++
    :start-after: /*begin_parallel_deterministic_reduce_example*/
    :end-before: /*end_parallel_deterministic_reduce_example*/
