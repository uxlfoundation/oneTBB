...
/*begin_parallel_reduce_imperative_example*/
...
float ParallelSum(const std::vector<float>& input) {
    Sum total;
    tbb::parallel_reduce(tbb::blocked_range<float*>(input.data(), input.data() + input.size()), total);
    return total.value;
}
/*end_parallel_reduce_imperative_example*/
