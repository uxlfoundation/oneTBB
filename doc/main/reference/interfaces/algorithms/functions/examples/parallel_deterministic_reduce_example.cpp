...
/*begin_parallel_deterministic_reduce_example*/
...
float ParallelSum(const std::vector<float>& input) {
    ...
    return tbb::parallel_deterministic_reduce(
        tbb::blocked_range<float*>(input.data(), input.data() + input.size(), grain_size),
        0.f,
...
/*end_parallel_deterministic_reduce_example*/
