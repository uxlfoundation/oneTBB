...
.. code:: cpp
    ...
    namespace tbb {
        ...
        public:
            ...
            task_group_status wait_for_task(task_completion_handle& ch);
            task_group_status run_and_wait_for_task(task_handle&& h);
            task_group_status get_status_of(task_completion_handle& ch);
...
* :ref:`Dynamic Dependencies<dynamic_dependencies>` - allows setting execution dependencies between tasks in
  ``task_group``.
* :ref:`Waiting for Individual Tasks<wait_single_task>` - allows waiting for an individual task to complete
  in ``task_group``.
