...
.. code:: cpp
    ...
    namespace flow {
        ...
        public:
            ...
            bool try_put_and_wait( const Input& v );
        };
...
The following :ref:`preview features<preview_features>` extend the ``function_node`` API:

* :ref:`Helper Functions for Expressing Graphs<helpers_for_expressing_graphs>` - allows ``function_node``
...
* :ref:`Waiting for Single Message in Flow Graph<try_put_and_wait>` - allows to put messages
  to ``function_node`` and wait for all related work to complete.
