...
.. code:: cpp
    ...
    namespace flow {
        ...
        public:
            ...
            bool try_put_and_wait( const T &v );
        };
...
The following :ref:`preview features<preview_features>` extend the ``priority_queue_node`` API:

* :ref:`Helper Functions for Expressing Graphs<helpers_for_expressing_graphs>` - allows ``priority_queue_node``
  to be constructed as a successor or a predecessor of the set of nodes.
* :ref:`Waiting for Single Message in Flow Graph<try_put_and_wait>` - allows to put messages
  to ``priority_queue_node`` and wait for all related work to complete.
