...
.. code:: cpp
    ...
    namespace flow {
        ...
        public:
            ...
            bool try_put_and_wait( const TupleType &v );
...
The following :ref:`preview features<preview_features>` extend the ``split_node`` API:

* :ref:`Helper Functions for Expressing Graphs<helpers_for_expressing_graphs>` - allows ``split_node``
...
* :ref:`Waiting for Single Message in Flow Graph<try_put_and_wait>` - allows to put messages
  to ``split_node`` and wait for all related work to complete.