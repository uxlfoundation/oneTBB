...
The following :ref:`preview features<preview_features>` extend the ``indexer_node`` API:

* :ref:`Helper Functions for Expressing Graphs<helpers_for_expressing_graphs>` - allows ``indexer_node``
...
* :ref:`Waiting for Single Message in Flow Graph<try_put_and_wait>` - allows to put messages
  to input ports of ``indexer_node`` and wait for all related work to complete.
