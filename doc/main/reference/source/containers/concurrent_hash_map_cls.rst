...
The following :ref:`preview features<preview_features>` extend the ``concurrent_hash_map`` API:

* :ref:`Customizing the Mutex Type for concurrent_hash_map<custom_mutex_chmap>` -
  allows setting the reader-writer mutex used by ``concurrent_hash_map`` to protect the concurrent operations.
