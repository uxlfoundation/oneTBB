...
The following preview features extend the ``write_once_node`` API:

* :ref:`Helper Functions for Expressing Graphs<helpers_for_expressing_graphs>` -
  allows ``write_once_node`` to be constructed as a successor or a predecessor of the set of nodes.
* :ref:`Waiting for Single Message in Flow Graph<waiting_for_single_message_in_flow_graph>` -
  allows to put messages to ``write_once_node`` and wait for all related work to complete.
