...
``task_completion_handle`` class represents a task for the purpose of setting execution
dependencies and tracking task completion. Unlike ``task_handle``, which becomes empty once
the task is submitted for execution, a ``task_completion_handle`` keeps referencing a task
...
A non-empty ``task_completion_handle`` can be constructed or assigned from a non-empty
``task_handle`` or copied from another non-empty ``task_completion_handle``.
