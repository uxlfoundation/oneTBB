...
public:
    successor_list_node(task_dynamic_state* successor, d1::small_object_allocator& alloc)
        : m_next_successor(nullptr)
        , m_dynamic_state(successor)
        , m_allocator(alloc)
    {}

    task_dynamic_state* get_dynamic_state() const {
...
private:
    ...
    task_dynamic_state* m_dynamic_state;
...
public:
    task_dynamic_state(task_handle_task* task, d1::small_object_allocator& alloc)
        ...
        , m_allocator(alloc)
        , m_num_predecessor_references(0)
    {}
    ...
    void add_predecessor_reference() {
        std::size_t prev = m_num_predecessor_references.fetch_add(1);
        if (prev == 0) {
            m_num_predecessor_references.fetch_add(1);
        }
    }

    task_handle_task* remove_predecessor_reference() {
        task_handle_task* next_task = nullptr;

        std::size_t prev = m_num_predecessor_references.fetch_sub(1);
        if (prev == 1) {
            next_task = m_task;
        }
        return next_task;
    ...
    bool has_dependencies() const {
        return m_num_predecessor_references.load(std::memory_order_acquire) != 0;
...
private:
    ...
    d1::small_object_allocator m_allocator;
    std::atomic<std::size_t> m_num_predecessor_references;
};
...
inline task_handle_task* release_successor_list(successor_list_node* node) {
    ...
    while (node != nullptr) {
        ...
        task_handle_task* successor_task = node->get_dynamic_state()->remove_predecessor_reference();
        node->finalize();
        node = next_node;
                                                             ...
                                                             successor_list_node* new_successor_node)
{
    ...
    if (is_completed(current_list_head)) {
        ...
        new_successor_node->get_dynamic_state()->remove_predecessor_reference();
...
inline void task_dynamic_state::add_successor(task_dynamic_state* successor) {
    ...
    if (!is_completed(current_successor_list_head)) {
        if (is_transferred(current_successor_list_head)) {
            ...
            successor->add_predecessor_reference();
            d1::small_object_allocator alloc;
...
inline void task_dynamic_state::release_continuation() {
    ...
    __TBB_ASSERT(m_num_predecessor_references.load(std::memory_order_relaxed) != 0, nullptr);
    task_handle_task* task = remove_predecessor_reference();
