...
| `-DTBB_VERIFY_DEPENDENCY_SIGNATURE=ON|OFF` | Controls signature verification of dynamic dependencies loaded during run-time | If set to ON, only signed dynamic dependecies will be loaded. This is recommended behavior. By default, the value of this option is unspecified, in which case the warning is printed. To suppress the warning set the value explicitly. |   |
