...
struct combine_task {
    ...
    std::shared_ptr<std::size_t> placeholder;
    std::shared_ptr<std::size_t> left_subleaf;
    std::shared_ptr<std::size_t> right_subleaf;
};
...
void test_successors_created(unsigned) {
    ...
    tbb::task_handle left_leaf_task = tg.defer(leaf_task{left_leaf_placeholder, task_initializer++});
    tbb::task_handle run_graph_leaf;

    std::shared_ptr<std::size_t> result_placeholder;
    ...
    CHECK_MESSAGE(*result_placeholder == 0, "Receiving results from incomplete task graph");

    // "Run" the graph
    tg.run_and_wait(std::move(run_graph_leaf));
    ...
    CHECK_MESSAGE(expected_result == *result_placeholder,
