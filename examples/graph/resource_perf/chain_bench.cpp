...
run_chain_bench(int num_executions, int num_inputs, int num_nodes, double generation_rate, int num_resources, int concurrency=1) {
...
#if USE_TRACE > 0
    std::unique_ptr<TraceCollector> trace_collector = make_trace_collector("chain_bench", num_executions, num_inputs, generation_rate, num_resources, concurrency);
#if USE_TRACE == 2
...
#endif
    // Register thread names for each node in the chain
...
int main(int argc, char* argv[]) {
    ...
    // Run benchmark
    auto [construction_time, execution_time] = run_chain_bench(num_executions, num_inputs, num_nodes, generation_rate, num_resources, concurrency);

    // Print results
    double total_graph_time_ms = num_nodes * cycle_sleep_time_ms;
    double delay_ms = total_graph_time_ms / generation_rate;
    print_results("chain_bench", construction_time, execution_time, num_executions, num_inputs,
